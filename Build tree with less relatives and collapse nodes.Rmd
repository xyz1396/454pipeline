...
df2<-read.csv("../table/E coli Nucleotide Similarity Matrix by IQ tree.csv")
distToStandard<-df2[df2$sseqid=="ASV2",c(1,2,4)]
...
# allEnLess.aligned.fasta  FASTA   DNA         51  108,120    2,120    2,120    2,120
seqkit stats ../temp/allEn.aligned.fasta
trimal -in ../temp/allEn.aligned.fasta -out ../temp/allEn.aligned.trimed.fasta -gt 0.20 -cons 40
# file                            format  type  num_seqs  sum_len  min_len  avg_len  max_len
# allEnLess.aligned.trimed.fasta  FASTA   DNA         51   21,624      424      424      424
seqkit stats ../temp/allEn.aligned.trimed.fasta
# check all sequence is unique after trim
seqkit rmdup ../temp/allEn.aligned.trimed.fasta -s -i -o ../temp/unique.fasta  \
  -d ../temp/duplicated.fasta -D ../temp/duplicated.detail.txt
mkdir ../temp/iqtree
iqtree -s ../temp/allEn.aligned.trimed.fasta -bb 1000 -redo -alrt 1000 \
...
p <- p + geom_fruit(
  ...
  theme(legend.justification = c(1, 0), legend.position = c(0.92, 0))
p + ggsave(
  "annotationDfLessRelatives 97 with quality rectangular by IQ-TREE.pdf",
  width = 8,
  height = 7
)
```
...
p2 <-
  ...
  collapse(61, 'min') %>% 
  collapse(90, 'min')
# p2+geom_hilight(node=90,fill="red")
p2<-p2+geom_cladelab(node=64, label="Serratia",hjust=-1.2)
p2<-p2+geom_cladelab(node=61, label="Serratia",hjust=-2.7)
p2<-p2+geom_cladelab(node=90, label="Serratia",hjust=-0.8)
p2<-p2+geom_cladelab(node=93, label="Salmonella",hjust=-0.8)
p2<-p2+geom_cladelab(node=69, label="Escherichia coli",hjust=0)
p2<-p2+xlim(0, 0.4)
segmentDf <- data.frame(x1 = 0.12,
                        x2 = 0.12,
                        y1 = 1,
                        y2 = 7)
# p2<-p2 + geom_segment(aes(
#   x = x1,
#   y = y1,
#   xend = x2,
#   yend = y2,
# ), data = segmentDf)
p2<-p2+annotate("segment", x=0.12,xend =0.12, y=1, yend = 7)
p2<-p2+annotate("text", x=0.15, y=4, label= "Pantoea")
p2<-p2+annotate("text", x=0.11, y=29, label= "Serratia")
p2 + ggsave(
  "Collapsing Clade.pdf",
  width = 8,
  height = 7
)
```
...
p2 <- p2 + geom_fruit(
  ...
  labs(fill = 'Similarity (%)') + 
  scale_fill_viridis(discrete = TRUE,option="B",na.translate = F)+
  theme(legend.justification = c(1, 0), legend.position = c(0.77, 0.23))
p2 <-
  collapse(p2, 93, 'min') %>% 
  collapse(64, 'min') %>% 
  collapse(61, 'min') %>% 
...
p2<-p2+annotate("segment", x=0.12,xend =0.12, y=1, yend = 7)
p2<-p2+annotate("text", x=0.15, y=4, label= "Pantoea")
p2<-p2+annotate("segment", x=0.263,xend =0.263, y=30, yend = 51)
p2<-p2+annotate("text", x=0.32, y=43, label= "Escherichia coli")
p2<-p2+annotate("text", x=0.11, y=29, label= "Serratia")
...
p2<-p2+geom_nodelab(size = 2,
               ...
               hjust = 0)
p2 + ggsave(
  "Collapsing Clade with abundance.pdf",
  width = 8,
  height = 7
)
```
