...
write.csv(df2,"../table/All Enterobacteriaceae Nucleotide Similarity by blast.csv",row.names = F)

# the alignment of E.coli Sequence
df3<-df2[str_detect(df2$qseqid,"ASV") & str_detect(df2$sseqid,"ASV"),]
nrow(df3)==19*19

# convert to matrix format
df4<-pivot_wider(df3[,1:3],names_from =sseqid,values_from =pident)
colnames(df4)[1]<-""
write.csv(df4,"../table/E coli Nucleotide Similarity Matrix by blast.csv",row.names = F)
```
